...
on_run(function()
    ...
    local files = {
        { TargetPath = path.join("Include/VulkanRenderer", classPath) .. ".hpp", Template = headerTemplate },
        { TargetPath = path.join("Include/VulkanRenderer", classPath) .. ".inl", Template = inlineTemplate }
    }

    if not option.get("nocpp") then
        table.insert(files, { TargetPath = path.join("Source/VulkanRenderer", classPath) .. ".cpp", Template = sourceTemplate })
    end
    ...
    local replacements = {
        ...
        HEADER_GUARD = "VKR_" .. classPath:gsub("[/\\]", "_"):upper() .. "_HPP"
    }
...
// This file is part of Vulkan Renderer.
...
#include <VulkanRenderer/%CLASS_PATH%.inl>
...
#include <VulkanRenderer/%CLASS_PATH%.hpp>
